...
static void sel_preview_draw(GtkDrawingArea *a, cairo_t *cr,
    ...
    if (st->dragging) {
        ...
        if (st->shape == 0) {
            ...
            double ww = fabs(st->cur_x - st->start_x);
            double hh = fabs(st->cur_y - st->start_y);
            cairo_rectangle(cr, x0, y0, ww, hh);
        } else if (st->shape == 1) {
            double cx = (st->start_x + st->cur_x) / 2.0;
            double cy = (st->start_y + st->cur_y) / 2.0;
            double rx = fabs(st->cur_x - st->start_x) / 2.0;
            double ry = fabs(st->cur_y - st->start_y) / 2.0;
            cairo_save(cr);
...
with open('src/modules/image/tools/image_layers.c', 'w') as f:
    f.write(c_code)
