...
static void set_result_ui(FolderDecryptCtx *ctx, const char *msg) {
    gtk_label_set_text(GTK_LABEL(ctx->result_lbl), msg);
}

static void start_crypto(FolderDecryptCtx *ctx) {
    if (!ctx->in_path) {
        set_result_ui(ctx, "Please choose a file to decrypt.");
        return;
    }
    if (!ctx->out_folder) {
        set_result_ui(ctx, "Please choose a destination folder.");
        return;
    }
    ...
    set_result_ui(ctx, "Processing... (This may take some time)");
    ...
    helv_crypto_result_t rc = helv_decrypt_folder(ctx->in_path, ctx->out_folder, password, kf_data, kf_len, &opts, NULL, NULL);
    ...
    if (rc == DECRYPT_OK) {
        char *msg = g_strdup_printf("✓ Folder decrypted successfully to %s", ctx->out_folder);
...
static void on_decrypt(GtkButton *btn, gpointer ud) { (void)btn; start_crypto((FolderDecryptCtx*)ud); }

static void on_reset(GtkButton *btn, gpointer ud) {
    (void)btn;
    FolderDecryptCtx *ctx = ud;
    g_free(ctx->in_path); ctx->in_path = NULL;
    g_free(ctx->keyfile_path); ctx->keyfile_path = NULL;
    g_free(ctx->out_folder); ctx->out_folder = NULL;
    
    gtk_label_set_text(GTK_LABEL(ctx->file_lbl), "No file selected.");
    gtk_label_set_text(GTK_LABEL(ctx->out_lbl), "No folder selected.");
...
static void update_preview(FolderDecryptCtx *ctx, const char *path) {
    char *basename = g_path_get_basename(path);
    FILE *in = g_fopen(path, "rb");
    if (in) {
        helv_header_t h;
        if (helv_read_header(in, &h) == DECRYPT_OK && h.container_type == HELV_CONTAINER_FOLDER) {
            char *size_str = g_format_size(h.original_size);
            char *info_txt = g_strdup_printf("%lu files | Size: %s", h.total_file_count, size_str);
            ...
            gtk_label_set_text(GTK_LABEL(ctx->preview_info_lbl), info_txt);
            gtk_label_set_text(GTK_LABEL(ctx->preview_status_lbl), (h.flags & 1) ? "Requires Key File" : "Ready to extract");
            
            g_free(size_str); g_free(info_txt);
            helv_header_free(&h);
        } else {
            gtk_widget_set_visible(ctx->preview_empty, FALSE);
            gtk_widget_set_visible(ctx->preview_box, TRUE);
            ...
            gtk_label_set_text(GTK_LABEL(ctx->preview_info_lbl), "Type: Invalid/Corrupt or not a folder");
...
static void on_out_picked(GObject *source, GAsyncResult *res, gpointer data) {
    FolderDecryptCtx *ctx = data;
    GError *err = NULL;
    GFile *f = gtk_file_dialog_select_folder_finish(GTK_FILE_DIALOG(source), res, &err);
    if (f) {
        g_free(ctx->out_folder);
        ctx->out_folder = g_file_get_path(f);
        char *base = g_path_get_basename(ctx->out_folder);
        gtk_label_set_text(GTK_LABEL(ctx->out_lbl), base);
        g_free(base);
        g_object_unref(f);
    }
}
...
static void on_browse(GtkButton *btn, gpointer ud) {
    ...
    gtk_file_dialog_set_title(dlg, "Choose encrypted folder (.helv)");
    gtk_file_dialog_open(dlg, NULL, NULL, on_file_picked, ud);
}

static void on_browse_out(GtkButton *btn, gpointer ud) {
    (void)btn;
    GtkFileDialog *dlg = gtk_file_dialog_new();
    gtk_file_dialog_set_title(dlg, "Choose destination folder");
    gtk_file_dialog_select_folder(dlg, NULL, NULL, on_out_picked, ud);
}
...
static GtkWidget* create_dec_pane(FolderDecryptCtx *ctx) {
    ...
    GtkWidget *title = gtk_label_new("Decrypt Folder");
    ...
    GtkWidget *file_row = create_custom_row("Encrypted folder", &ctx->file_lbl, G_CALLBACK(on_browse), ctx);
    adw_preferences_group_add(ADW_PREFERENCES_GROUP(group), file_row);
    
    GtkWidget *out_row = create_custom_row("Restore to", &ctx->out_lbl, G_CALLBACK(on_browse_out), ctx);
    adw_preferences_group_add(ADW_PREFERENCES_GROUP(group), out_row);
    
    GtkWidget *keyfile_row = create_custom_row("Key file (Optional)", &ctx->keyfile_lbl, G_CALLBACK(on_browse_keyfile), ctx);
    ...
    adw_expander_row_add_row(ADW_EXPANDER_ROW(expander), create_check_row("Restore permissions", TRUE, &ctx->chk_perms));
    adw_expander_row_add_row(ADW_EXPANDER_ROW(expander), create_check_row("Restore ownership", FALSE, &ctx->chk_owner));
    adw_expander_row_add_row(ADW_EXPANDER_ROW(expander), create_check_row("Restore timestamps", TRUE, &ctx->chk_times));
    adw_expander_row_add_row(ADW_EXPANDER_ROW(expander), create_check_row("Overwrite existing", FALSE, &ctx->chk_overwrite));
    ...
    ctx->btn = gtk_button_new_with_label("Decrypt Folder");
...
static GtkWidget* create_preview_pane(FolderDecryptCtx *ctx) {
    GtkWidget *box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 16);
    ...
    ctx->preview_info_lbl = gtk_label_new("");
    ...
    gtk_box_append(GTK_BOX(ctx->preview_box), ctx->preview_info_lbl);
...
GtkWidget *build_folder_decrypt(void) {
    FolderDecryptCtx *ctx = g_new0(FolderDecryptCtx, 1);
